...
def watson_request(file_index):
	mydir = os.getcwd()
	print ("Openening:" + (mydir + "/%d.wav" % file_index))
	fileMutex.acquire()
	audio = open((mydir + "/%d.wav" % file_index), 'rb')
	fileMutex.release()
...
def doWork():
	while True:
		file_index = q.get()
		watson_request(file_index)
		# doSomethingWithResult(status, url)
		print("TASK DONE WOHOOOOOO " + str(file_index))
		q.task_done()
...
def main():
	...
	rename_wav(".", "tmp.wav")
	# audio = sys.argv[1]
	wav_length = get_wav_length('./tmp.wav')
	print(wav_length)
	splits = get_split_times(wav_length)

	for i, split in enumerate(splits):
		output_path = str(i) + ".wav"
		crop_wav(int(split[0]), int(split[1]), "tmp.wav", (str(i) + ".wav"))
		#write_recording(i)

	num_threads = len(splits) - 1
	print("NUMBER OF SPLITS")
	print(num_threads)
